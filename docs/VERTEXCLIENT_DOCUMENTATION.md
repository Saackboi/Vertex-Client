...
- GET /api/onboarding/resume: { currentStep: number, serializedData: string, isCompleted: boolean }
- POST /api/onboarding/save: { currentStep: number, serializedData: string } con serializedData = JSON.stringify(formData)
...
- OnboardingService: getResume(), saveProgress(dto), complete(); maneja serializedData como string.
...
- Onboarding (/onboarding): NzSteps + formularios reactivos; al iniciar GET /resume para prefilling; en Siguiente POST /save con serializedData string; muestra éxito (nz-result) al completar.
...
- Onboarding: init GET /resume (si isCompleted redirigir a dashboard; si 404 empezar paso 1); cada paso POST /save con serializedData string; al final POST /save y opcional /complete.

### 11) Reglas y notas
- Solo Ng-Zorro (sin Material/Bootstrap). Importar módulos por componente standalone.
- serializedData siempre string; no enviar objetos planos ni userId.
...
- JWT in Authorization: Bearer <token> (interceptor). POST /api/auth/login { email, password } -> { token, email, fullName, expiresAt }. GET /api/onboarding/resume returns { currentStep, serializedData, isCompleted }. POST /api/onboarding/save expects { currentStep, serializedData: JSON.stringify(formData) }. Do not send userId; use ApiResponse<T> semantics.

### Services
- AuthService (token/user storage, signals isAuthenticated, currentUser$). OnboardingService (getResume, saveProgress, complete) using serializedData string. AuthInterceptor adds header, clears storage and routes to /login on 401.
...
### Rules
- Ng-Zorro only; standalone imports per component. serializedData is string; never send plain objects or userId. Use environment values for URLs/keys. Keep this doc updated when contracts/routes/layout change.
